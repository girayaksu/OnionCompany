...
using OnionCompany.Application.ManagerInterfaces;
...
namespace OnionCompany.InnerInfrastructure.ManagerConcretes
{
    public abstract class BaseManager<T, U> : IManager<T, U> where T : class, IDto where U : class, IEntity
    {
        ...
        public async System.Threading.Tasks.Task<string> HardDeleteAsync(int id)
        {
            U value = await _repository.GetByIdAsync(id);
        ...
        public async System.Threading.Tasks.Task<string> SoftDeleteAsync(int id)
        {
            U value = await _repository.GetByIdAsync(id);
